...
public class main {
  public static void main(String[] args) {
    ...
    ArrayList<ArrayList<Integer>> clear = new ArrayList<>();

    for(int i = 0; i < arr.length; i++){
      String level [] = arr[i].split(" ");
      ArrayList<Integer> clearLevel = new ArrayList<>();

      for(int j = 0; j < level.length; j++){
        clearLevel.add(Integer.parseInt(level[j]));
      }
      clear.add(clearLevel);
    }
    System.out.println("");
    ...
    outer:
    for (ArrayList<Integer> level : clear) {
      for(int i = 0; i < level.size(); ++i){
        ArrayList<Integer> a = new ArrayList<>(level);
        a.remove(i);
        if(check(a)){
          countValid ++;
          continue outer;
  ...
  public static boolean check(ArrayList<Integer> a){
    float sign = Math.signum(a.get(0) - a.get(1));

    for (int i = 0; i < a.size() - 1; i++) {
      float diff = a.get(i) - a.get(i + 1);
      float currentSign = Math.signum(diff);

      if ((sign != currentSign) || (Math.abs(diff) < 1) || (Math.abs(diff) > 3)) {
        return false;
      }
    }
    return true;
